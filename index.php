            ...
            if($type == 'create_complaint') {
                ...
                if(!empty($m_arr[1]) && !empty($m_arr[2])) {
                    ...
                    if(strlen($m_arr[1]) == 9) {
                        ...
                        if($q == 0) {
                            $message_t = 'Ошибка добавления заявки. - ' . $m_arr . ' - ' . json_encode($m_arr) . ' - ' . json_encode($array) . ' - ' . $message;
                        } elseif($q == 2) {
                            $message_t = 'В xDrive нет консультанта с регистрационным номером ' . $m_arr[1];
                        } else {
                            $message_t = 'Вашей заявке присвоен №' . $q . '. Мы отправим вам ответ в ближайшее время.' . json_encode($q);
                        }
                    } else {
